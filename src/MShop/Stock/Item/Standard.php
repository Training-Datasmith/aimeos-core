 ...
 * @subpackage Stock
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Stock\Item\Iface
{
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'stock.productid': $item->setProductId($value);
                    break;
                case 'stock.stocklevel': $item->setStockLevel($value);
                    break;
                case 'stock.timeframe': $item->setTimeFrame($value);
                    break;
                case 'stock.dateback': $item->setDateBack($value);
                    break;
                case 'stock.type': $item->setType($value);
                    break;
                default: continue 2;
            }
