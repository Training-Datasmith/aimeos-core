 ...
 * @subpackage Locale
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Locale\Item\Site\Iface
{
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'locale.site.code': $item->setCode($value);
                    break;
                case 'locale.site.label': $item->setLabel($value);
                    break;
                case 'locale.site.status': $item->setStatus((int) $value);
                    break;
                case 'locale.site.config': $item->setConfig((array) $value);
                    break;
                case 'locale.site.refid': $item->setRefId($value);
                    break;
                case 'locale.site.logo': $item->setLogos((array) $value);
                    break;
                case 'locale.site.theme': $item->setTheme($value);
                    break;
                case 'locale.site.icon': $item->setIcon($value);
                    break;
                default: continue 2;
            }
