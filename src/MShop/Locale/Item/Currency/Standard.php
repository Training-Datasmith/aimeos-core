 ...
 * @subpackage Locale
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Locale\Item\Currency\Iface
{
    /**
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'locale.currency.code': $item->setCode($value);
                    break;
                case 'locale.currency.label': $item->setLabel($value);
                    break;
                case 'locale.currency.status': $item->setStatus((int) $value);
                    break;
                default: continue 2;
            }
