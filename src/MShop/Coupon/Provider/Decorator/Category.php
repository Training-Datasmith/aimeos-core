 ...
 * @subpackage Coupon
 */
class Category extends \Aimeos\M_Shop\Coupon\Provider\Decorator\Base implements \Aimeos\M_Shop\Coupon\Provider\Decorator\Iface
{
    private array $be_config = ['category.code' => ['code' => 'category.code', 'internalcode' => 'category.code', 'label' => 'Comma separated category codes', 'default' => '', 'required' => true], 'category.only' => ['code' => 'category.only', 'internalcode' => 'category.only', 'label' => 'Rebate is applied only to products of that category', 'type' => 'bool', 'default' => false, 'required' => false]];
    /**
    ...
    {
        if ($this->get_config_value('category.only') == true) {
            $prod_ids = [];
            $context = $this->context();
            $types = ['default', 'promotion'];
            $cat_manager = \Aimeos\M_Shop::create($context, 'catalog');
            $prod_manager = \Aimeos\M_Shop::create($context, 'product');
            $codes = explode(',', $this->get_config_value('category.code', ''));
            $filter = $cat_manager->filter(true)->add(['catalog.code' => $codes])->slice(0, count($codes));
            ...
            $filter->add($filter->is($filter->make('product:has', ['catalog', $types, $cat_ids]), '!=', null))->add($filter->is('product.id', '==', array_keys($prod_ids)));
    ...
    {
        if (($codes = $this->get_config_value('category.code')) !== null) {
            $expr = [];
            $context = $this->context();
            $types = ['default', 'promotion'];
            $cat_manager = \Aimeos\M_Shop::create($context, 'catalog');
            $prod_manager = \Aimeos\M_Shop::create($context, 'product');
            $filter = $cat_manager->filter(true)->add(['catalog.code' => explode(',', $codes)]);
            $cat_ids = $cat_manager->search($filter)->keys()->all();
            $filter = $prod_manager->filter(true);
            foreach ($order->get_products() as $product) {
                $expr[] = $filter->is($filter->make('product:has', ['catalog', $types, $cat_ids]), '!=', null);
            }
