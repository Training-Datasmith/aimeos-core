 ...
 * @subpackage Coupon
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Coupon\Item\Iface
{
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'coupon.label': $item->setLabel($value);
                    break;
                case 'coupon.datestart': $item->setDateStart($value);
                    break;
                case 'coupon.dateend': $item->setDateEnd($value);
                    break;
                case 'coupon.provider': $item->setProvider($value);
                    break;
                case 'coupon.status': $item->setStatus((int) $value);
                    break;
                case 'coupon.config': $item->setConfig((array) $value);
                    break;
                default: continue 2;
            }
