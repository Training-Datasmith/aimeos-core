 ...
 * @subpackage Coupon
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Coupon\Item\Code\Iface
{
    /**
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'coupon.code.parentid': !$private ?: $item->setParentId($value);
                    break;
                case 'coupon.code.datestart': $item->setDateStart($value);
                    break;
                case 'coupon.code.dateend': $item->setDateEnd($value);
                    break;
                case 'coupon.code.count': $item->setCount($value);
                    break;
                case 'coupon.code.code': $item->setCode($value);
                    break;
                case 'coupon.code.ref': $item->setRef($value);
                    break;
                default: continue 2;
            }
