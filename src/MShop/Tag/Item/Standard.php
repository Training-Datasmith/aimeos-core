 ...
 * @subpackage Tag
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Tag\Item\Iface
{
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'tag.languageid': $item->setLanguageId($value);
                    break;
                case 'tag.domain': $item->setDomain($value);
                    break;
                case 'tag.label': $item->setLabel($value);
                    break;
                case 'tag.type': $item->setType($value);
                    break;
                default: continue 2;
            }
