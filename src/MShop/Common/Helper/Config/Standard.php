                ...
                switch ($attr->getType()) {
                    ...
                    case 'boolean':
                        if (!in_array($config[$key], ['', '0', '1'])) {
                            $errors[$key] = sprintf('Not a true/false value');
                            continue 2;
                        }
                        break;
                    case 'string':
                    case 'text':
                        if (!is_string($config[$key]) && !is_numeric($config[$key])) {
                            $errors[$key] = sprintf('Not a string');
                            continue 2;
                        }
                        break;
                    case 'int':
                    case 'integer':
                        if (!is_integer($config[$key]) && !(is_string($config[$key]) && ctype_digit($config[$key]))) {
                            $errors[$key] = sprintf('Not an integer number');
                            continue 2;
                        }
                        break;
                    case 'number':
                        if (!is_numeric($config[$key])) {
                            $errors[$key] = sprintf('Not a number');
                            continue 2;
                        }
                        break;
                    case 'date':
                    ...
                    case 'datetime':
                        ...
                        if (!is_string($config[$key]) || preg_match($pattern, $config[$key]) !== 1) {
                            $errors[$key] = sprintf('Not a date and time');
                            continue 2;
                        }
                        break;
                    case 'time':
                    ...
                    case 'map':
                        if (!is_array($config[$key])) {
                            $errors[$key] = sprintf('Not a key/value map');
                            continue 2;
                        }
                        break;
                    default:
