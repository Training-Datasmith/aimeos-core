 ...
 * @subpackage Common
 */
class Standard extends \Aimeos\M_Shop\Common\Item\Base implements \Aimeos\M_Shop\Common\Item\Lists\Iface
{
    use \Aimeos\M_Shop\Common\Item\Config\Traits;
    use \Aimeos\M_Shop\Common\Item\Type_Ref\Traits;
    private string $date;
    private string $prefix;
    private ?\Aimeos\M_Shop\Common\Item\Iface $ref_item = null;
    /**
    ...
    {
        return $this->get_domain() . '|' . $this->get_type() . '|' . $this->get_ref_id();
    }
    /**
    ...
    public function to_array(bool $private = false): array
    {
        ...
        $list[$this->prefix . 'domain'] = $this->get_domain();
        $list[$this->prefix . 'refid'] = $this->get_ref_id();
        $list[$this->prefix . 'datestart'] = $this->get_date_start();
        $list[$this->prefix . 'dateend'] = $this->get_date_end();
        $list[$this->prefix . 'config'] = $this->get_config();
        $list[$this->prefix . 'position'] = $this->get_position();
        $list[$this->prefix . 'status'] = $this->get_status();
