 ...
 * @subpackage Common
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Common\Item\Lists\Iface
{
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case $this->prefix . 'parentid': !$private ?: $item->setParentId($value);
                    break;
                case $this->prefix . 'domain': $item->setDomain($value);
                    break;
                case $this->prefix . 'type': $item->setType($value);
                    break;
                case $this->prefix . 'refid': $item->setRefId($value);
                    break;
                case $this->prefix . 'datestart': $item->setDateStart($value);
                    break;
                case $this->prefix . 'dateend': $item->setDateEnd($value);
                    break;
                case $this->prefix . 'status': $item->setStatus((int) $value);
                    break;
                case $this->prefix . 'config': $item->setConfig((array) $value);
                    break;
                case $this->prefix . 'position': $item->setPosition((int) $value);
                    break;
                default: continue 2;
            }
