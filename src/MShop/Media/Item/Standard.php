 ...
 * @subpackage Media
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Media\Item\Iface
{
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'media.filesystem': $item->setFileSystem($value);
                    break;
                case 'media.domain': $item->setDomain($value);
                    break;
                case 'media.label': $item->setLabel($value);
                    break;
                case 'media.languageid': $item->setLanguageId($value);
                    break;
                case 'media.mimetype': $item->setMimeType($value);
                    break;
                case 'media.type': $item->setType($value);
                    break;
                case 'media.url': $item->setUrl($value);
                    break;
                case 'media.preview': $item->setPreview($value);
                    break;
                case 'media.previews': $item->setPreviews((array) $value);
                    break;
                case 'media.status': $item->setStatus((int) $value);
                    break;
                default: continue 2;
            }
