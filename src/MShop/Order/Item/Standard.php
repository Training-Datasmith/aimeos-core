 ...
 * @subpackage Order
 */
class Standard extends \Aimeos\MShop\Order\Item\Base implements \Aimeos\MShop\Order\Item\Iface
{
    /**
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'order.channel': !$private ?: $item->setChannel($value);
                    break;
                case 'order.invoiceno': !$private ?: $item->setInvoiceNumber($value);
                    break;
                case 'order.statusdelivery': !$private ?: $item->setStatusDelivery((int) $value);
                    break;
                case 'order.statuspayment': !$private ?: $item->setStatusPayment((int) $value);
                    break;
                case 'order.datedelivery': !$private ?: $item->setDateDelivery($value);
                    break;
                case 'order.datepayment': !$private ?: $item->setDatePayment($value);
                    break;
                case 'order.customerid': !$private ?: $item->setCustomerId($value);
                    break;
                case 'order.customerref': $item->setCustomerReference($value);
                    break;
                case 'order.languageid': $item->locale()->setLanguageId($value);
                    break;
                case 'order.relatedid': $item->setRelatedId($value);
                    break;
                case 'order.comment': $item->setComment($value);
                    break;
                default: continue 2;
            }
