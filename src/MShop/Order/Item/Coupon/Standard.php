 ...
 * @subpackage Order
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Order\Item\Coupon\Iface
{
    /**
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'order.coupon.parentid': !$private ?: $item->setParentId($value);
                    break;
                case 'order.coupon.productid': !$private ?: $item->setProductId($value);
                    break;
                case 'order.coupon.code': $item->setCode($value);
                    break;
                default: continue 2;
            }
