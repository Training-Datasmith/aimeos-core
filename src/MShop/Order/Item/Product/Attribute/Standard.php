 ...
 * @subpackage Order
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Order\Item\Product\Attribute\Iface
{
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'order.product.attribute.attributeid': !$private ?: $item->setAttributeId($value);
                    break;
                case 'order.product.attribute.parentid': !$private ?: $item->setParentId($value);
                    break;
                case 'order.product.attribute.siteid': !$private ?: $item->setSiteId($value);
                    break;
                case 'order.product.attribute.type': $item->setType($value);
                    break;
                case 'order.product.attribute.code': $item->setCode($value);
                    break;
                case 'order.product.attribute.name': $item->setName($value);
                    break;
                case 'order.product.attribute.value': $item->setValue($value);
                    break;
                case 'order.product.attribute.price': $item->setPrice($value);
                    break;
                case 'order.product.attribute.quantity': $item->setQuantity($value);
                    break;
                default: continue 2;
            }
