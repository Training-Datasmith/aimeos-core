 ...
 * @subpackage Order
 */
class Standard extends \Aimeos\MShop\Order\Item\Status\Base implements \Aimeos\MShop\Order\Item\Status\Iface
{
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'order.status.parentid': !$private ?: $item->setParentId($value);
                    break;
                case 'order.status.type': $item->setType($value);
                    break;
                case 'order.status.value': $item->setValue($value);
                    break;
                default: continue 2;
            }
