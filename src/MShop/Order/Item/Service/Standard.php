...
class Standard extends Base implements Iface
{
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'order.service.siteid': !$private ?: $item->setSiteId($value);
                    break;
                case 'order.service.parentid': !$private ?: $item->setParentId($value);
                    break;
                case 'order.service.serviceid': !$private ?: $item->setServiceId($value);
                    break;
                case 'order.service.type': $item->setType($value);
                    break;
                case 'order.service.code': $item->setCode($value);
                    break;
                case 'order.service.name': $item->setName($value);
                    break;
                case 'order.service.currencyid': $price = $price->setCurrencyId($value);
                    break;
                case 'order.service.price': $price = $price->setValue($value);
                    break;
                case 'order.service.costs': $price = $price->setCosts($value);
                    break;
                case 'order.service.rebate': $price = $price->setRebate($value);
                    break;
                case 'order.service.taxrates': $price = $price->setTaxRates($value);
                    break;
                case 'order.service.taxvalue': $price = $price->setTaxValue($value);
                    break;
                case 'order.service.taxflag': $price = $price->setTaxFlag($value);
                    break;
                case 'order.service.position': $item->setPosition($value);
                    break;
                case 'order.service.mediaurl': $item->setMediaUrl($value);
                    break;
                default: continue 2;
            }
