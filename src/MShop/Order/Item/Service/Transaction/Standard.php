 ...
 * @subpackage Order
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Order\Item\Service\Transaction\Iface
{
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'order.service.transaction.parentid': !$private ?: $item->setParentId($value);
                    break;
                case 'order.service.transaction.siteid': !$private ?: $item->setSiteId($value);
                    break;
                case 'order.service.transaction.config': $item->setConfig((array) $value);
                    break;
                case 'order.service.transaction.status': $item->setStatus((int) $value);
                    break;
                case 'order.service.transaction.currencyid': $price->setCurrencyId($value);
                    break;
                case 'order.service.transaction.type': $item->setType($value);
                    break;
                case 'order.service.transaction.price': $price->setValue($value);
                    break;
                case 'order.service.transaction.costs': $price->setCosts($value);
                    break;
                case 'order.service.transaction.rebate': $price->setRebate($value);
                    break;
                case 'order.service.transaction.taxvalue': $price->setTaxvalue($value);
                    break;
                case 'order.service.transaction.taxflag': $price->setTaxflag((bool) $value);
                    break;
                default: continue 2;
            }
