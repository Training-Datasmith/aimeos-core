 ...
 * @subpackage Subscription
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Subscription\Item\Iface
{
    /**
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'subscription.orderid': $item->setOrderId($value);
                    break;
                case 'subscription.ordprodid': $item->setOrderProductId($value);
                    break;
                case 'subscription.productid': $item->setProductId($value);
                    break;
                case 'subscription.datenext': $item->setDateNext($value);
                    break;
                case 'subscription.dateend': $item->setDateEnd($value);
                    break;
                case 'subscription.interval': $item->setInterval($value);
                    break;
                case 'subscription.period': $item->setPeriod((int) $value);
                    break;
                case 'subscription.status': $item->setStatus((int) $value);
                    break;
                case 'subscription.reason': $item->setReason($value !== null ? (int) $value : null);
                    break;
                default: continue 2;
            }
