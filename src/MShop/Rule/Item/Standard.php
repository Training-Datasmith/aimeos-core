 ...
 * @subpackage Rule
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Rule\Item\Iface
{
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'rule.type': $item->setType($value);
                    break;
                case 'rule.label': $item->setLabel($value);
                    break;
                case 'rule.provider': $item->setProvider($value);
                    break;
                case 'rule.status': $item->setStatus((int) $value);
                    break;
                case 'rule.config': $item->setConfig((array) $value);
                    break;
                case 'rule.position': $item->setPosition((int) $value);
                    break;
                case 'rule.datestart': $item->setDateStart($value);
                    break;
                case 'rule.dateend': $item->setDateEnd($value);
                    break;
                default: continue 2;
            }
