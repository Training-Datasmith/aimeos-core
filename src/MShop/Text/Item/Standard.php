 ...
 * @subpackage Text
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Text\Item\Iface
{
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'text.languageid': $item->setLanguageId($value);
                    break;
                case 'text.type': $item->setType($value);
                    break;
                case 'text.label': $item->setLabel($value);
                    break;
                case 'text.domain': $item->setDomain($value);
                    break;
                case 'text.content': $item->setContent($value);
                    break;
                case 'text.status': $item->setStatus((int) $value);
                    break;
                default: continue 2;
            }
