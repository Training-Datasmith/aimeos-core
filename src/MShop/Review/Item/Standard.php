 ...
 * @subpackage Review
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Review\Item\Iface
{
    /**
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'review.orderproductid': !$private ?: $item->setOrderProductId($value);
                    break;
                case 'review.customerid': !$private ?: $item->setCustomerId($value);
                    break;
                case 'review.refid': $item->setRefId($value);
                    break;
                case 'review.domain': $item->setDomain($value);
                    break;
                case 'review.comment': $item->setComment($value);
                    break;
                case 'review.response': $item->setResponse($value);
                    break;
                case 'review.status': $item->setStatus((int) $value);
                    break;
                case 'review.rating': $item->setRating((int) $value);
                    break;
                case 'review.name': $item->setName($value);
                    break;
                default: continue 2;
            }
