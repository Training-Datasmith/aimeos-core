 ...
 * @subpackage Supplier
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Supplier\Item\Iface
{
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'supplier.code': $item->setCode($value);
                    break;
                case 'supplier.label': $item->setLabel($value);
                    break;
                case 'supplier.status': $item->setStatus((int) $value);
                    break;
                case 'supplier.position': $item->setPosition((int) $value);
                    break;
                default: continue 2;
            }
