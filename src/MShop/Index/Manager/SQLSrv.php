 ...
 * @subpackage Index
 */
class Sql_Srv extends \Aimeos\M_Shop\Index\Manager\Standard implements \Aimeos\M_Shop\Index\Manager\Iface, \Aimeos\M_Shop\Common\Manager\Factory\Iface
{
    ...
    {
        return $this->get_sub_manager_base('index', $manager, $name ?: 'SQLSrv');
    }
    /**
    ...
    protected function get_sub_managers(): array
    {
        if ($this->sub_managers === null) {
            ...
            foreach ($config->get('mshop/index/manager/submanagers', []) as $domain) {
                ...
                $this->sub_managers[$domain] = $this->object()->get_sub_manager($domain, $name ?: 'SQLSrv');
            }
