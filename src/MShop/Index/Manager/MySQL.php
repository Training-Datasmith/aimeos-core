 ...
 * @subpackage Index
 */
class My_Sql extends \Aimeos\M_Shop\Index\Manager\Standard implements \Aimeos\M_Shop\Index\Manager\Iface, \Aimeos\M_Shop\Common\Manager\Factory\Iface
{
    ...
    {
        return $this->get_sub_manager_base('index', $manager, $name ?: 'MySQL');
    }
    /**
    ...
    protected function get_sub_managers(): array
    {
        if ($this->sub_managers === null) {
            ...
            foreach ($config->get('mshop/index/manager/submanagers', []) as $domain) {
                ...
                $this->sub_managers[$domain] = $this->object()->get_sub_manager($domain, $name ?: 'MySQL');
            }
