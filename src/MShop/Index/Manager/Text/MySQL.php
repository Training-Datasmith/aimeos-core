 ...
 * @subpackage Index
 */
class My_Sql extends \Aimeos\M_Shop\Index\Manager\Text\Standard
{
    private array $search_config = ['index.text:relevance' => ['code' => 'index.text:relevance()', 'internalcode' => ':site AND mindte."langid" = $1 AND MATCH( mindte."content" ) AGAINST( $2 IN BOOLEAN MODE )', 'label' => 'Product texts, parameter(<language ID>,<search term>)', 'type' => 'float'], 'sort:index.text:relevance' => ['code' => 'sort:index.text:relevance()', 'internalcode' => 'MATCH( mindte."content" ) AGAINST( $2 IN BOOLEAN MODE ) * mpro."boost"', 'label' => 'Product text sorting, parameter(<language ID>,<search term>)', 'type' => 'float', 'public' => false]];
    /**
    ...
    public function __construct(\Aimeos\M_Shop\Context_Iface $context)
    {
        ...
        $sql = $this->search_config['index.text:relevance']['internalcode'];
        $this->search_config['index.text:relevance']['internalcode'] = str_replace(':site', $expr, $sql);
        $this->search_config['sort:index.text:relevance']['function'] = $func;
