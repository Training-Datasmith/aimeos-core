 ...
 * @subpackage Type
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MShop\Type\Item\Iface
{
    /**
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Type\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case $prefix . 'code': $item->setCode($value);
                    break;
                case $prefix . 'domain': $item->setDomain($value);
                    break;
                case $prefix . 'i18n': $item->setI18n((array) $value);
                    break;
                case $prefix . 'label': $item->setLabel($value);
                    break;
                case $prefix . 'position': $item->setPosition((int) $value);
                    break;
                case $prefix . 'status': $item->setStatus((int) $value);
                    break;
                default: continue 2;
            }
