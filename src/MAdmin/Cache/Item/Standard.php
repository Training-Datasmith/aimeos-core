 ...
 * @subpackage Cache
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MAdmin\Cache\Item\Iface
{
    /**
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'cache.id': !$private ?: $item->setId($value);
                    break;
                case 'cache.value': $item->setValue($value);
                    break;
                case 'cache.expire': $item->setTimeExpire($value);
                    break;
                case 'cache.tags': $item->setTags($value);
                    break;
                default: continue 2;
            }
