 ...
 * @subpackage Job
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MAdmin\Job\Item\Iface
{
    /**
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'job.path': $item->setPath((string) $value);
                    break;
                case 'job.label': $item->setLabel((string) $value);
                    break;
                case 'job.status': $item->setStatus((int) $value);
                    break;
                default: continue 2;
            }
