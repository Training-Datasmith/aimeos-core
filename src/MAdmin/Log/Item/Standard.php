 ...
 * @subpackage Log
 */
class Standard extends \Aimeos\MShop\Common\Item\Base implements \Aimeos\MAdmin\Log\Item\Iface
{
    /**
    ...
    public function fromArray(array &$list, bool $private = false): \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach ($list as $key => $value) {
            switch ($key) {
                case 'log.facility': $item->setFacility($value);
                    break;
                case 'log.priority': $item->setPriority($value);
                    break;
                case 'log.message': $item->setMessage($value);
                    break;
                case 'log.request': $item->setRequest($value);
                    break;
                default: continue 2;
            }
