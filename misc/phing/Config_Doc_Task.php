    ...
    protected function sanitize(array $options)
    {
        ...
        foreach ($options as $key => $values) {
            if (strncmp($key, $this->prefix, $len) !== 0) {
                ...
                unset($options[$key]);
            } elseif (!isset($values['short']) && $key[0] !== "\n") {
                $this->log('No doc: ' . $key);
            }
        }
